...
export function applyCSG(
  ...
  try {
    ...
    customGeometry: resultMesh.geometry, // Add the resulting geometry
        customMaterial: Array.isArray(resultMesh.material) ? resultMesh.material[0] : resultMesh.material, // Ensure single Material
        createdAt: Date.now(),
        updatedAt: Date.now(),
      }
    }
...
function getMeshFromObject(obj: ModelObject): THREE.Mesh | null {
  ...
  switch (obj.type) {
    ...
    case "sphere":
      ...
      geometry = new THREE.SphereGeometry(obj.radius ?? obj.scale[0] / 2 ?? 0.5, 32, 32)
      break
    case "cylinder":
