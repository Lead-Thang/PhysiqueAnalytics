...
export function HybridToolbar({
    ...
    const handleMouseMove = useCallback(
        (e: MouseEvent) => {
            ...
            const newX = Math.max(0, Math.min(window.innerWidth - toolbarWidth, e.clientX - dragOffset.x));
            const newY = Math.max(0, Math.min(window.innerHeight - toolbarHeight, e.clientY - dragOffset.y));
